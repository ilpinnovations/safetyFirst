...
/**
 * Created by 1094048
 * This class is generating Excell file which can be used for data display.
 * GenerateExcel
 */
public class GenerateExcel extends AsyncTask<Void, Void, Void> {
    /**
     ...
     */
    public GenerateExcel(Survey survey,ArrayList<GroupItem> groupItemsArrayList, File file) {
    ...
    public void generateExcel() throws IOException, WriteException {
      ...
      //          Log.v("AAAAAfileExist","asfadf");
       ...
       //             String ref_no = childItem.getTitle().substring(0,1);
                    ...
                    sheet.addCell(new Label(3, k, "NA", cellFormat_grey));
                    WritableCell c1 = sheet.getWritableCell(1,k);
                    WritableCell c2 = sheet.getWritableCell(2,k);
                    c1.setCellFormat(cellFormat_grey_wrap);
                    c2.setCellFormat(cellFormat_grey);
                    sheet.setColumnView(0, 150);
                    sheet.addCell(new Label(4, k,childItem.getDescription(),cellFormat_grey_wrap));
